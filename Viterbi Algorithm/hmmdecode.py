...
with open(train_file_path,'r') as fopen:
    for line in fopen:
        backpointer_prob_map=defaultdict(dict)
        last_transition_map={}
        previous_word_tags=[]
            ...
            if word_list[i] in word_tag_emission_count_map:
                ...
                if i == 0:
                    for tag, prob in tag_cnt_map.iteritems():
                        ...
                        backpointer_prob_map[i+ 1][tag_transition_key] = math.log(transition_probability) + math.log(emission_probability)
                else:
                    tag_appended_string=''
                    for tag, prob in tag_cnt_map.iteritems():
                        for previous_tag in previous_word_tags:
                            ...
                            last_transition_map = backpointer_prob_map.get(i)
                            max_prob = -10000000
                            ...
                            backpointer_prob_map[i+1][max_prob_transition_key] = max_prob
                    previous_word_tags = []
                ...
                if i==0:
                    ...
                    for tag in total_tag_list:
                        ...
                        backpointer_prob_map[i+ 1][next_trans_key] = next_trans_prob
                else:
                    last_transition_map = backpointer_prob_map.get(i)
                    for tag_tran_key, prob1 in last_transition_map.iteritems():
        ...
        for i in range(len(word_list),0,-1):
            tag_transitions_for_word = backpointer_prob_map.get(i)
            if i == len(word_list):
